...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    connect(colorDialog, &ColorDialog::currentColorChanged, [=](const QColor &color) {
        ...
        ui->textBrowser->setText("<body style='text-align:center;color:" + color.name() +
                                 ...
                                 "<p>" +
                                 ui->textBrowser->getParaText() +
                                 "</p>"
                                 "</body>");
        QString fontColor = QColor("white").lighter(-color.lightness() + 300).name();
        ui->pattern_generator->setStyleSheet("text-align:left;color:" + fontColor +
                                             ";padding:4px;border:none;background-color: "
    ...
    foreach (QLabel *label, colorDialog->findChildren<QLabel *>())
    {
        ...
        if (label->text().contains("Custom colors"))
        {
            ...
            label->setText(
                "<b>Temporary Swatch</b><br>Click \"Add to Temporary Swatch\"<br> to save color permanently. <br>or<br>"
...
void MainWindow::add_to_table(const QString colorStr, bool saving)
{
    ...
    if (color.isValid())
    {
        ...
        for (int i = 0; i < columnData.count(); i++)
        {
            ...
            {
                ui->saved->setItem(nextRow, i, new QTableWidgetItem(columnData.at(i)));
            }
        }
    }
    else
...
void MainWindow::on_pattern_generator_clicked()
{
    ...
    if (color.isValid())
    {
        ...
        for (int i = 0; i < itemscount; i++)
        {
            ...
            connect(colorWidget, &QPushButton::clicked,
                    [colorNameHash, this]() { colorDialog->setCurrentColor(QColor(colorNameHash.trimmed())); });
